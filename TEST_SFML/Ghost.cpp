...
float Ghost::get_target_distance(unsigned char i_direction)
{
	...
	//Because I know math.
	//Are you impressed?
	// pow을 통해 2제곱, sqrt를 통해 제곱근을 구해준다,
...
void Ghost::update(unsigned char i_level, std::array<std::array<Cell, MAP_HEIGHT>, MAP_WIDTH>& i_map, Ghost& i_ghost_0, Pacman& i_pacman)
{
	...
	// 5초마다 프레임을 한 장씩 줄어든다 == 속도 빨라진다. 프레임 개수 1이 되면 더이상 줄이지 않는다.	
	if (start_count_time == 0)
	{
		start_count_time = 1;
		t1 = clock();
	}
	...
	{
		t2 = clock();
		if ((t2 - t1) > (5 * CLOCKS_PER_SEC))
		{
			if (GHOST_FRIGHTENED_SPEED != 1)
			{
	...
	{
		frightened_speed_timer = GHOST_DONT_SEE;

		frightened_mode = 1;
	}
	else if (0 == i_pacman.get_energizer_timer() && 1 == frightened_mode) // 시간이 다됐다.
	{
		frams_speed = GHOST_DONT_SEE;
	...
	if (0 == frams_speed)
	{
		...
		frams_speed = GHOST_FRIGHTENED_SPEED;
